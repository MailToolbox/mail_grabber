var MailGrabberApplication = {
   ...
   *
   * @param {Number} messageId - which message would we like to delete
   */
  ...
  /**
   * Delete the message list when we click on the Clear tab. It will remove
   * everything.
   */
   ...
   *
   * @param {Number} messageId - which message would we like to see
   */
   ...
   * so that we can know which page we are on. It checks the message ids to not
   * load those messages that are already on the list.
   */
  ...
  /**
   * Scrolling infinitely. Count the height of the list, and if we reach the
   * bottom of the list, then try to load more messages.
   *
  ...
  /**
   * Load the message list. Also, add an event listener for infinite scroll.
   */
   ...
   * we scroll down or click on the Reload tab.
   */
   ...
   * @param {String} path - the path that we can get/send data
   * @param {Function} fn - the function that handles the response
   */
